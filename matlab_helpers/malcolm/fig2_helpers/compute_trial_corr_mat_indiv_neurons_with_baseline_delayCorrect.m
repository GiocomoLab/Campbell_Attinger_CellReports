function [corrmat,shiftmat,rep_info,corrmat_bl,shiftmat_bl,rep_info_bl] = compute_trial_corr_mat_indiv_neurons_with_baseline_delayCorrect(session_name,cell_info,paths,opt,sensory_delay)
...
parfor i = 1:numel(session_name)
    ...
    % find sensory delay for each cell
    uniq_cell_id = cell(numel(good_cells),1);
    delay_factors = nan(numel(good_cells),1);
    for iCell = 1:numel(good_cells)
        uniq_cell_id{iCell} = sprintf('%s_c%d',session{1},good_cells(iCell));
        idx = find(strcmp(sensory_delay.UniqID,uniq_cell_id{iCell}));
        if ~isempty(idx)
            delay_factors(iCell) = sensory_delay.DelayFactor(idx);
        end
    end
    good_cells = good_cells(~isnan(delay_factors));
    delay_factors = delay_factors(~isnan(delay_factors));
    
    % make sure at least enough cells remaining
    if numel(good_cells)<opt.min_num_stab_cells
        continue;
    end
    
    % correct spike times by sensory delay (different for each cell)
    for iCell = 1:numel(good_cells) 
        dat.sp.st(dat.sp.clu==good_cells(iCell)) = dat.sp.st(dat.sp.clu==good_cells(iCell))+delay_factors(iCell);
    end
