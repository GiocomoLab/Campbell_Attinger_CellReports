...
if opt.match_depth
    hfig.Name = 'Figure S5C middle: mean fr, depth, and intercell dist_match depth';
elseif opt.match_fr
    hfig.Name = 'Figure S5C bottom: mean fr, depth, and intercell dist_match fr';
else
    hfig.Name = 'Figure S5C top: mean fr, depth, and intercell dist_no match';
end
...
plot(x1,f1,'b');
plot(x2,f2,'k');
if ranksum(y(:,1),y(:,2))<0.001
    title('***');
elseif ranksum(y(:,1),y(:,2))<0.01
    title('**');
elseif ranksum(y(:,1),y(:,2))<0.05
    title('*');
else
    title('n.s.');
end
