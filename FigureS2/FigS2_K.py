...
if __name__=='__main__':
    ...
    files = ['AA1_190726_gain_1.mat', #V1
    'AA46_190920_gaincontrast10_1.mat', #RSC
    'npF4_1023_gaincontrast_1.mat', #MEC
    'npF4_1025_gaincontrast_2.mat'] #MEC
    

    im_path =os.path.join(data_dir,'umap_gain08_fig')
    ...
    for fi in files:
        try:
            ...
            # gain_tmp[-3]=0.6
            # gain_tmp[-4]=0.5
            # sc=ax.scatter(X_um[:,0],X_um[:,1],c=gain_tmp,s=3,cmap='Set2')
            gain_idx = np.arange(1200,2000)
            bl_idx = np.arange(0,1200)
            bl_post = np.arange(2000,3200)
            import pdb
            # pdb.set_trace()
            ax.plot(X_um[bl_idx,0],X_um[bl_idx,1],'k.',)
            ax.plot(X_um[gain_idx,0],X_um[gain_idx,1],'.',color='#7AFFDE') ##C92BFF
            ax.plot(X_um[bl_post,0],X_um[bl_post,1],'.',color=(0.4,0.4,0.4))
